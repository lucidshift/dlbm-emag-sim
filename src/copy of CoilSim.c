...
#define u0 1 //MPERM
#define u  3119
 //MPERM * 3119		//Technically this is relative permeability when u0 is anything other than 4pi * 10^-7 
#define SIZEX 4000
			...
			for(int z= v.zLow; z< v.zHigh; z++){
				for(int i=0; i<velCount; i++){
					...
					A[s][i][x][y][z] = A[s][i][x][y][z] 
										+ ((1./(u0 * alpha)) * ((velDist[i] * rho[s][x][y][z]) - A[s][i][x][y][z])) 
										+ (Source[s][x][y][z] / alpha) * velDist[i] * sourceOn;
				}
			}
		}
	}
      	...
      	for (int y=1;y<YDIM-1;y++){
			for(int z=1; z<(ZDIM-1); z++){
				...
				//B field is calculated and redimensionalized here.
      			B[x][y][z][0] = -1000 * 0.0148809524 * ((rho[2][x][y][z] - rho[2][x][y-1][z]) - (rho[1][x][y][z] - rho[1][x][y][z-1])) * (muTemp * MPERM);
      			B[x][y][z][1] = -1000 * 0.0148809524 * ((rho[0][x][y][z] - rho[0][x][y][z-1]) - (rho[2][x][y][z] - rho[2][x-1][y][z])) * (muTemp * MPERM);
      			B[x][y][z][2] = -1000 * 0.0148809524 * ((rho[1][x][y][z] - rho[1][x-1][y][z]) - (rho[0][x][y][z] - rho[0][x][y-1][z])) * (muTemp * MPERM);
				...
				*/								
				...
				H[x][y][z][0] = B[x][y][z][0] / (mu[x][y][z] * MPERM);
				H[x][y][z][1] = B[x][y][z][1] / (mu[x][y][z] * MPERM);
				H[x][y][z][2] = B[x][y][z][2] / (mu[x][y][z] * MPERM);
						...
						for(int i=-3; i<4; i++){
							for(int j=-3; j<4; j++){
								for(int k=-3; k<4; k++){
									...
									if(x!=xx || y!=yy){
										distance = sqrt(((x-xx+xOff)*(x-xx+xOff)) + ((y-yy+yOff)*(y-yy+yOff)) + ((z-zz+zOff)*(z-zz+zOff))) * 0.000000001;
									}
											
									//Does not currently have alpha factored in it, seems to break when that's the case.					
									rhoTh[x][y] += mu[xx][yy][zz] * MPERM * (Source[s][xx][yy][zz] / distance) / (3.1415926 * 1./(mu[x][y][z]));  // (J/d) / (4pi * 0.25 * M) 
								}
							}
						}
					}
...
void GetGraph(int s){
	...
	if(sliceReq){
		sliceReq=0;
		for(int x=0; x<XDIM; x++){
			slice[x] = 1000 * rho[s][x][sliceLoc][zSlice];			
	...
	for(int x=0; x<XDIM; x++){
		for(int y=0; y<YDIM; y++){
			for(int z=0; z<ZDIM; z++){
				...
				EMFDisplay[x][y][0] = JMagnitude * J[x][y][zSlice].x + JInduced[0][x][y][zSlice]; //Includes source current 
				EMFDisplay[x][y][1] = JMagnitude * J[x][y][zSlice].y + JInduced[1][x][y][zSlice];
	...
	for(int x=0; x<XDIM; x++){
		for(int y=0; y<YDIM; y++){
			for(int z=0; z<ZDIM; z++){
				...
				eTotal += (((B[x][y][z][0] * B[x][y][z][0]) + (B[x][y][z][1]*B[x][y][z][1]) + (B[x][y][z][2]*B[x][y][z][2])) * 0.000000001) / (2 * mu[x][y][z] * MPERM);
...
void CalcInducedEMF(){
	...
	double muFactor, dxMu, dyMu, dzMu;
		...
		for(int y=1; y<YDIM-1; y++){
			for(int z=1; z<ZDIM-1; z++){
				...
				*/
				...
				JInduced[0][x][y][z] = ((dxMu * dxRhoX) + (dyMu * dxRhoY) + (dzMu * dxRhoZ));// * muFactor;
				JInduced[1][x][y][z] = ((dxMu * dyRhoX) + (dyMu * dyRhoY) + (dzMu * dyRhoZ));// * muFactor;
				JInduced[2][x][y][z] = ((dxMu * dzRhoX) + (dyMu * dzRhoY) + (dzMu * dzRhoZ));// * muFactor;	
