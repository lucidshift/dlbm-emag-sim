...
#define Ydim 64
#define tubedim 50
#define curve3ddim 100

static int sinsinreq,sinexpreq,sinreq,cosreq,creq,vecreq,vec2req,treq,tubereq;
...
static double curve3d[curve3ddim][3];
static int no3dcurve=curve3ddim,curve3dreq=0;
static  double sinexpdata[Xdim*Ydim];
static  double vecdata[Xdim*Ydim*2],*vecdata2;
...
void myvec2(int x, int y, double *res){
  res[0]=cos(1.0*(x*y+xtime)/100);
  res[1]=-sin(1.0*(x*y+xtime)/120);
}
...
void providedata() { 
  ...
  if (vec2req){ 
    for (x=0;x<Xdim;x++)
      for (y=0;y<Ydim;y++)
	myvec2(x,y,&vecdata2[(x*Ydim+y)*2]);
    vec2req=0;
  }
  if (treq){ 
    for (x=0;x<Xdim;x++)
  ...
  if (curve3dreq){
    curve3dreq=0;
    for (x=0;x<curve3ddim;x++){
      curve3d[x][0] = sin(8*3.14*x/curve3ddim * sin(0.001*xtime));
      curve3d[x][1] = cos(8*3.14*x/curve3ddim * sin(0.001*xtime));
      curve3d[x][2] = cos(8*3.14*x/curve3ddim);
    }
  }
  if (Cont1req){
    DefineContour();
...
void Draw(int xdim, int ydim){
  ...
  mydrawline(3,0,ydim,xdim,0);
  mycircle(2,xdim/3,ydim/3,xdim/10);
  myselectfont(0,"test",xdim/10);
  mytext(1,3*xdim/4,3*ydim/4,"Hello World",0);
}
...
int main(int argc,char **argv){
  ...
  vecdata2=malloc(Xdim*Ydim*2*sizeof(double));

  AddFreedraw("Simple drawing",&Draw);
  sindatap=&sindata[0];
  SetDefaultColor(1);
  SetDefaultShape(3);
  SetDefaultFill(1);
  ...
  DefineGraphN_RxRxRxR("tube",&tube[0][0],&notube,&tubereq);
  ...
  DefineGraphNxN_RxRp("(cos[x],sin[Y])",&vecdata2,
                   &vecxdim,&vecydim,&vec2req);
  DefineGraphNxN_RxRxRt("(Sin[x],Sin[x],Cos[Y])",&tdata[0],
                   &txdim,&tydim,&treq);
  DefineGraphContour3d("(wuerfel)",&Cont1,
                   &Contxdim,&Contydim,&Contzdim,&Cont1req);
  DefineGraphN_RxRxR("3d curve",&(curve3d[0][0]),&no3dcurve,&curve3dreq);
