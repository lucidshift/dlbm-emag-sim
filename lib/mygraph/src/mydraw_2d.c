...
void myXfontsinit(void)
{
  ...
  int i=0,j,nofoundfonts=0;
  const char *teststr="X";
  char **allfonts;
  char fontbasename[10][200] = {"times","clean","helvetica","fixed","terminal","avantgarde","lucida","palatino","bookman","courier"};
  char fontpattern[300];

  if (initialized==0){
    initialized=1;
    while (nofoundfonts==0){
      if (i>=10){
	printf("Fonts could not be initialized [mydraw_2d.c, myXfonstinit()]");
	return;
      }
      sprintf(fontpattern, "*%s-medium-r-*", fontbasename[i++]);
